...
const getDefaultConfig = require('./configuration/defaults')
...
class Momentum {
  ...
  async initialize(config = {}) {
    ...
    if (this.config.startUpMaxWaitMilliseconds !== 0) {
      startTimeout = setTimeout(() => {
        if (!this.ready) {
          let diff = Date.now() - startTime
          this.fatal(`timeout waiting for Momentum to become ready... (waited: ${diff/1000} sec; maxWait: ${this.config.startUpMaxWaitMilliseconds/1000} sec)`)
        }
      }, this.config.startUpMaxWaitMilliseconds || MAX_WAIT_TIME)
    }
  ...
  async loadConfiguration() {
    ...
    return loadConfiguration(this, defaultConfig)
  }
