...
class Api {
  ...
  loadHandlers() {
    ...
    let rootLength = (route) => {
      return route.options.root.length
    }

    let routes = _.sortBy(_.values(momentum.routes), [rootLength])
    _.each(routes, (route) => {
      _.extend(paths, route.getApiPaths())
      _.extend(handlers, route.getApiHandlers())
    })

    let controllers = _.sortBy(_.values(momentum.controllers), [rootLength])
    _.each(controllers, (controller) => {
      _.extend(paths, controller.getApiPaths())
      _.extend(handlers, controller.getApiHandlers())
    })

    let pathOperations = _.reduce(_.values(paths), (memo, pathValues) => {
      let ids = _.map(_.values(pathValues), 'operationId')
      return memo.concat(_.compact(ids))
    }, [])

    let operationIds = _.keys(handlers)
    let unusedHandlers = _.difference(operationIds, pathOperations)

    _.each(unusedHandlers, (key) => {
      delete handlers[key]
    })
  }
