...
function addMomentumMethods(q) {
  q.populate = function (associationNames) {
    q.associationNames = q.associationNames || []

    if (_.isString(associationNames)) {
      associationNames = [associationNames]
    }

    if (_.isArray(associationNames)) {
      q.associationNames = _.uniq(q.associationNames.concat(associationNames))
    }
...
function exec(Model, q, eventName, args) {
  ...
  q.exec = async function _exec() {
    ...
    let associationPromises = _(models)
      .map((model) => {
        return _.map(q.associationNames, (associationName) => {
          return model.populate(associationName)
        })
      })
      .flatten()
      .value()
...
function execOne(Model, q, eventName, args) {
  ...
  q.exec = async function _execOne() {
    ...
    let associationPromises = _.map(q.associationNames, (associationName) => {
      return model.populate(associationName)
    })
    await Promise.all(associationPromises)
...
class TableModel {
  ...
  getAssociation(associationName) {
    if (!this.Model.associations[associationName]) {
      throw new Error(`Association ${associationName} does not exist`)
    }
