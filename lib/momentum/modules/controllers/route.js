...
class Route extends Responder {
  ...
  createApiPath(path, action) {
    ...
    return this[action.key] && action.verb !== 'ws' ? apiPath : {}
  }
  ...
  getSocketListeners() {
    ...
    _.each(this.actions, (action) => {
      let path = `${this.basePath}${action.path}` || '/'
      if (this[action.key] && action.verb === 'ws') {
        listeners[path] = this.createWsHandler(action.key, action.options)
      }
    })
